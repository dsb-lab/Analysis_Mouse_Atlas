...
def loadData(name="./dataOut/Raw.h5ad",QC_basic=False,QC_imputed=False,Normalized=False,Log1p=False,QC_Doublets_Stripped=False,Log=False,sample="All",stage="All"):
    ...
    if QC_imputed:
        a = a[np.invert(a.obs.loc[:,"QC_basic_trimmed"]),np.invert(a.var.loc[:,"QC_basic_trimmed"])]
        a = a[np.invert(a.obs.loc[:,"imputed"]),:]
        
    if Normalized:
        scp.pp.normalize_total(a,target_sum=meanCount)
...
def remove_highly_variable_genes(a,genes,genelabel="Gene",highly_variable_label="highly_variable"):
    for i in genes:
        a.var.loc[a.var[genelabel]==i,highly_variable_label] = False
